...
This can be translated to English as "the list integers whose values are
```x``` raised to the power of three where ```x``` is taken from the list we
...
As we might expect, the native Erlang versiosn is much more concise.

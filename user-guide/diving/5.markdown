...
several example usages of receive are given there. On the first page of the
tutorial, any message sent to ```receive``` is accepted. Let's replace the
simple pattern of the whole data (i.e., ```msg```) with a series of patterns.

...
We've saved the best special variable to last: ```$ENV```. When you first
start up a shell, the ```$ENV``` variable holds pristine state data:
{% highlight cl %}
> $ENV
(#(variable *** ())
 #(variable ** ())
...
There is, as you might have guessed, much more that is displayed, mostly macro
definitions. This can come in very handy when debugging the loading of files,
macros, etc.
